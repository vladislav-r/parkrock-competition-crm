...
@router.patch("/event/public-refresh", response_model=EventRead,
              dependencies=[Depends(require_permission(Permission.publication_manage))])
def update_public_refresh(payload: PublicRefreshUpdate, operation_id: OperationId,
    ...
    record, replay = begin_operation(
        db, operation_id=operation_id, admin_id=admin.id, action="event.public-refresh.update",
