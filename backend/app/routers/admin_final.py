...
from app.models import (
    Admin, AgeGroup, Event, EventStage, QualificationCategorySnapshot,
...
def status_response(db: Session, event: Event) -> FinalStatusResponse:
    ...
    categories = [QualificationCategoryStatus(
        ...
        confirmed_at=group.qualification_confirmed_at, expected_version=group.version,
    ) for group in groups]
    ...
    return FinalStatusResponse(
        event_id=event.id, stage=event.stage, final_started_at=event.final_started_at,
...
def recalculate_final_category(db: Session, category_snapshot_id: uuid.UUID, route_ids: list[uuid.UUID]) -> None:
    ...
    rows: list[tuple[FinalCategoryResult, tuple[int, int, int, int, int], bool]] = []
    for result in results:
        ...
        zone_attempts = sum(item.zone_attempt for item in route_attempts if item and item.top_attempt is None and item.zone_attempt is not None)
        has_result = any(item is not None for item in route_attempts)
        values = (score, tops, zones, top_attempts, zone_attempts)
        if (result.score_tenths, result.top_count, result.zone_count, result.top_attempts, result.zone_attempts) != values:
            result.score_tenths, result.top_count, result.zone_count, result.top_attempts, result.zone_attempts = values
        rows.append((result, values, has_result))
    for result, _, has_result in rows:
        if not has_result:
            result.place = None
    ranked_rows = [item for item in rows if item[2]]
    ranked_rows.sort(key=lambda item: (-item[1][0], -item[1][1], -item[1][2], item[1][3], item[1][4]))
...
def cancel_final(
    ...
    record, replay = begin_operation(
        db, operation_id=operation_id, admin_id=admin.id, action="final.cancel-development",
        target_type="event", target_id=str(event.id), payload=payload.model_dump(),
    )
    if replay is not None:
        return replay
