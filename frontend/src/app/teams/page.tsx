...
import { ApiError, getPublicResults, getTeamResults, TeamResults, TeamStage } from "@/lib/api";
...
export default function TeamsPage() {
  ...
  const load = useCallback(async () => {
    try { const publicData = await getPublicResults(); const teams = await getTeamResults(stage, undefined, publicData.publication_version); setData(teams); setGroups(publicData.groups); setRefresh(publicData); setError(""); }
    catch (reason) { if (reason instanceof ApiError && reason.status === 404) { setData(null); setGroups([]); } setError(reason instanceof Error ? reason.message : "Не удалось загрузить командный зачёт"); }
  }, [stage]);
