...
const IMPORT_COLUMNS = ["Фамилия", "Имя", "Отчество", "Год рождения", "Пол", "Разряд", "Сет", "Футболка"];
const OPTIONAL_IMPORT_COLUMNS = new Set(["Отчество", "Футболка"]);
...
export function ParticipantImportDialog({ token, onClose, onImported, onNotice }: {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="import-dialog stage3-import-dialog" role="dialog" aria-modal="true" aria-labelledby="import-title" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {!preview && <><p>Загрузите шаблон коллективной заявки. «Название команды» и представитель обязательны, телефон и футболка — необязательны. Участники появятся в системе только после вашего подтверждения.</p><div className="import-columns">{IMPORT_COLUMNS.map((column) => <span key={column} className={OPTIONAL_IMPORT_COLUMNS.has(column) ? "optional-column" : ""}>{column}{OPTIONAL_IMPORT_COLUMNS.has(column) && <small>необязательно</small>}</span>)}</div><label className={busy ? "file-upload-button disabled" : "file-upload-button"}><Upload size={17}/>{busy ? "Проверяем файл..." : "Выбрать и проверить файл"}<input type="file" accept=".xlsx" disabled={busy} onChange={(event) => { const selected = event.target.files?.[0]; if (selected) void inspect(selected); }}/></label></>}
...
export function ParticipantCreateDialog({ token, event, onClose, onCreated, onNotice }: {
  ...
  function prepare(formData: FormData) {
    ...
    setPending({
      ...
      representative: String(formData.get("representative")).trim(), merch_size: String(formData.get("merch_size")).trim() || null,
    });
  }
