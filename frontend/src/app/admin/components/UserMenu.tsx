...
export function UserMenu({ user, token, onGuide, onSettings, onLogout }: { user: CurrentUser; token: string; onGuide: () => void; onSettings?: () => void; onLogout: () => void }) {
  ...
  return <details className="admin-user-menu" ref={ref} onToggle={event => setOpen(event.currentTarget.open)}>
    ...
    <div className="admin-user-dropdown">
      {open && user.permissions.includes("users.presence") && <OnlineStaff token={token} currentUserId={user.id}/>}
