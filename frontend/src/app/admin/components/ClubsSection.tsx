...
import { ArrowDownUp, CreditCard, Info, Merge, MoreVertical, Pencil, Search, UserCheck, Users, X } from "lucide-react";
...
export function ClubsSection({ token, stage, canEdit, canMerge, canManage, onParticipantsChanged }: { token: string; stage?: EventInfo["stage"]; canEdit: boolean; canMerge: boolean; canManage: boolean; onParticipantsChanged: () => void }) {
  ...
  const clubsPage = usePagination(visibleClubs.length, search);
  const membersPage = usePagination(visibleMembers.length, JSON.stringify([selectedClubId, memberSearch, sort]));
  const pageMembers = visibleMembers.slice(membersPage.offset, membersPage.offset + membersPage.pageSize);
  const clubsRef = useRef<HTMLDivElement>(null);
  const membersRef = useRef<HTMLDivElement>(null);
  useEffect(() => { clubsRef.current?.scrollTo({ top: 0 }); }, [clubsPage.offset, clubsPage.pageSize, search]);
  useEffect(() => { membersRef.current?.scrollTo({ top: 0 }); }, [membersPage.offset, membersPage.pageSize, selectedClubId, memberSearch, sort]);
  ...
  return <section className="clubs-workspace">
    <header className="admin-section-hero"><div><h1>Клубы</h1><p>{clubs.length} клубов · {clubs.reduce((sum, item) => sum + item.participant_count, 0)} участников</p></div></header>
    <div className="clubs-content-grid">
    <aside className="clubs-list-pane"><div className="clubs-pane-head"><label className="search-box"><Search size={17}/><input data-view-action value={search} onChange={(event) => setSearch(event.target.value)} aria-label="Найти клуб" placeholder="Найти клуб"/></label></div><div className="clubs-list" ref={clubsRef}>{visibleClubs.slice(clubsPage.offset, clubsPage.offset + clubsPage.pageSize).map((item) => <div className={item.id === selectedClubId ? "club-list-entry active" : "club-list-entry"} key={item.id}><button data-view-action className={item.id === selectedClubId ? "club-list-item active" : "club-list-item"} onClick={() => { setSelectedClubId(item.id); setDetailOpen(true); }}><span><strong>{item.name}</strong><small>{item.representative || "Представитель не указан"}</small></span><b>{item.participant_count}</b></button>{(canMerge || canEdit) && <div className="club-menu"><button className="club-menu-trigger" aria-label={`Действия клуба ${item.name}`} popoverTarget={`club-menu-${item.id}`} onClick={(event) => {
      const rect = event.currentTarget.getBoundingClientRect();
      const popup = document.getElementById(`club-menu-${item.id}`);
      if (popup) { popup.style.left = `${Math.max(8, Math.min(rect.right - 170, window.innerWidth - 178))}px`; popup.style.top = `${Math.min(rect.bottom + 4, window.innerHeight - 160)}px`; }
    }}><MoreVertical size={18}/></button><div id={`club-menu-${item.id}`} popover="auto" className="club-menu-popup">{canEdit && <button onClick={(event) => { (event.currentTarget.parentElement as HTMLElement).hidePopover(); openClubEditor(item); }}><Pencil size={15}/>Редактировать клуб</button>}{canMerge && <button onClick={(event) => { (event.currentTarget.parentElement as HTMLElement).hidePopover(); setMergeSource(item); }}><Merge size={15}/>Объединить</button>}{canEdit && canManage && <button className="danger-menu-item" disabled={stage !== "preparation"} title="Удаление доступно только на этапе подготовки" onClick={(event) => { (event.currentTarget.parentElement as HTMLElement).hidePopover(); setDeleting({ club: item, operationId: crypto.randomUUID() }); }}><Trash2 size={15}/>Удалить</button>}</div></div>}</div>)}</div><ListPagination {...clubsPage} label="Клубы"/></aside>
    <ResponsiveDetail open={detailOpen && Boolean(club)} onClose={() => setDetailOpen(false)} label="Карточка клуба"><div className="club-detail-pane">{club ? <><div className="club-relief-body"><div className="club-detail-head"><div><h2>{club.name}</h2><p>Представитель: {club.representative || "не указан"}</p></div><div className="club-summary"><span><b>{club.checked_in_count}/{club.participant_count}</b><small>прибыли</small></span><span><b>{club.paid_count}/{club.participant_count}</b><small>оплатили</small></span></div></div>
      ...
      {canManage && <div className="club-bulk-actions"><button className="reception-confirm" onClick={() => queueBulk("checked_in", true)}><UserCheck size={15}/>Подтвердить прибытие</button><button onClick={() => queueBulk("checked_in", false)}><X size={15}/>Отменить прибытие</button><button className="reception-confirm" onClick={() => queueBulk("is_paid", true)}><CreditCard size={15}/>Отметить оплату</button><button onClick={() => queueBulk("is_paid", false)}><X size={15}/>Отменить оплату</button></div>}
      <div className="club-member-search"><label className="search-box"><Search size={17}/><input data-view-action value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} aria-label="Поиск участников клуба" placeholder="Номер или ФИО участника"/></label><span>{visibleMembers.length} из {club.members.length}</span></div>
      </div>
      <div className="club-members-table" ref={membersRef}><div className="club-member-row table-head"><label className="member-check"><input type="checkbox" aria-label="Выбрать всех показанных участников" checked={pageMembers.length > 0 && pageMembers.every((member) => selectedIds.has(member.id))} ref={(input) => { if (input) input.indeterminate = pageMembers.some((member) => selectedIds.has(member.id)) && !pageMembers.every((member) => selectedIds.has(member.id)); }} onChange={(event) => { const checked = event.target.checked; setSelectedIds((current) => { const next = new Set(current); pageMembers.forEach((member) => checked ? next.add(member.id) : next.delete(member.id)); return next; }); }}/></label>{([["start_number", "№"], ["full_name", "ФИО"], ["set_name", "Сет"], ["application_type", "Заявка"], ["checked_in", "Прибытие"], ["is_paid", "Оплата"]] as Array<[SortKey, string]>).map(([key, label]) => <button data-view-action key={key} className={sort.key === key ? "active" : ""} onClick={() => changeSort(key)}>{label}<ArrowDownUp size={12}/></button>)}</div>{pageMembers.map((member) => <div className="club-member-row" key={member.id}><label className="member-check"><input type="checkbox" aria-label={`Выбрать участника №${member.start_number}`} checked={selectedIds.has(member.id)} onChange={() => toggleMember(member.id)}/><span/></label><b className="member-number">№{member.start_number}</b><strong className="member-full-name">{member.full_name}</strong><span>{member.set_name}</span><span>{member.application_type === "collective" ? "Коллективная" : "Индивидуальная"}</span><button disabled={!canManage} className={member.checked_in ? "status-chip success" : "status-chip"} onClick={() => setPending({ scope: "individual", member, field: "checked_in", value: !member.checked_in })}>{member.checked_in ? "Прибыл" : "Не прибыл"}</button><button disabled={!canManage} className={member.is_paid ? "status-chip success" : "status-chip"} onClick={() => setPending({ scope: "individual", member, field: "is_paid", value: !member.is_paid })}>{member.is_paid ? "Оплачено" : "Не оплачено"}</button></div>)}</div><ListPagination {...membersPage} label="Состав клуба"/>
