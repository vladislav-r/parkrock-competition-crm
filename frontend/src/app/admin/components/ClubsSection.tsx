...
import { ArrowDownUp, CreditCard, Info, Merge, MoreVertical, Pencil, Search, UserCheck, Users } from "lucide-react";
...
export function ClubsSection({ token, canEdit, canMerge, canManage, onParticipantsChanged }: { token: string; canEdit: boolean; canMerge: boolean; canManage: boolean; onParticipantsChanged: () => void }) {
  ...
  async function saveClub() {
    ...
    setSaving(true);
    try {
      const result = await updateClub(token, club.id, clubDraft.name.trim(), clubDraft.representative.trim(), club.version);
      setEditingClub(false); setSelectedClubId(result.id); await load(); onParticipantsChanged();
  ...
  return <section className="clubs-workspace">
    ...
    <aside className="clubs-list-pane"><div className="clubs-pane-head"><label className="search-box"><Search size={17}/><input value={search} onChange={(event) => setSearch(event.target.value)} aria-label="Найти клуб" placeholder="Найти клуб"/></label></div><div className="clubs-list">{visibleClubs.map((item) => <div className={item.id === selectedClubId ? "club-list-entry active" : "club-list-entry"} key={item.id}><button className={item.id === selectedClubId ? "club-list-item active" : "club-list-item"} onClick={() => setSelectedClubId(item.id)}><span><strong>{item.name}</strong><small>{item.representative || "Представитель не указан"}</small></span><b>{item.participant_count}</b></button>{canMerge && <div className="club-menu"><button className="club-menu-trigger" aria-label={`Действия клуба ${item.name}`} popoverTarget={`club-menu-${item.id}`} onClick={(event) => {
      const rect = event.currentTarget.getBoundingClientRect();
      const popup = document.getElementById(`club-menu-${item.id}`);
      if (popup) { popup.style.left = `${Math.max(8, Math.min(rect.right - 170, window.innerWidth - 178))}px`; popup.style.top = `${Math.min(rect.bottom + 4, window.innerHeight - 60)}px`; }
    }}><MoreVertical size={18}/></button><div id={`club-menu-${item.id}`} popover="auto" className="club-menu-popup"><button onClick={(event) => { (event.currentTarget.parentElement as HTMLElement).hidePopover(); setMergeSource(item); }}><Merge size={15}/>Объединить</button></div></div>}</div>)}</div></aside>
    <div className="club-detail-pane">{club ? <><div className="relief-card-ribbon">Клуб</div><div className="club-relief-body"><div className="club-detail-head"><div><h2>{club.name}</h2><p>Представитель: {club.representative || "не указан"}</p></div><div className="club-summary"><span><b>{club.checked_in_count}/{club.participant_count}</b><small>прибыли</small></span><span><b>{club.paid_count}/{club.participant_count}</b><small>оплатили</small></span></div></div>
      <div className="club-selection-toolbar"><div className="club-selection-note" title="Индивидуальных участников можно добавить вручную."><Info size={16}/><strong>Выбрано: {selectedMembers.length} · Коллективные заявки: {club.collective_count}</strong></div>{canEdit && <button className="edit-club-button" onClick={openClubEditor}><Pencil size={14}/>Редактировать клуб</button>}</div>
      <div className="club-table-toolbar">
      {canManage && <div className="club-bulk-actions"><button onClick={() => queueBulk("checked_in", true)}><UserCheck size={15}/>Подтвердить прибытие</button><button onClick={() => queueBulk("checked_in", false)}>Отменить прибытие</button><button onClick={() => queueBulk("is_paid", true)}><CreditCard size={15}/>Отметить оплату</button><button onClick={() => queueBulk("is_paid", false)}>Отменить оплату</button></div>}
      <div className="club-member-search"><label className="search-box"><Search size={17}/><input value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} aria-label="Поиск участников клуба" placeholder="Номер или ФИО участника"/></label><span>{visibleMembers.length} из {club.members.length}</span></div>
      </div>
      <div className="club-members-table"><div className="club-member-row table-head"><label className="member-check"><input type="checkbox" aria-label="Выбрать всех показанных участников" checked={visibleMembers.length > 0 && visibleMembers.every((member) => selectedIds.has(member.id))} ref={(input) => { if (input) input.indeterminate = visibleMembers.some((member) => selectedIds.has(member.id)) && !visibleMembers.every((member) => selectedIds.has(member.id)); }} onChange={(event) => { const checked = event.target.checked; setSelectedIds((current) => { const next = new Set(current); visibleMembers.forEach((member) => checked ? next.add(member.id) : next.delete(member.id)); return next; }); }}/></label>{([["start_number", "№"], ["full_name", "ФИО"], ["set_name", "Сет"], ["application_type", "Заявка"], ["checked_in", "Прибытие"], ["is_paid", "Оплата"]] as Array<[SortKey, string]>).map(([key, label]) => <button key={key} className={sort.key === key ? "active" : ""} onClick={() => changeSort(key)}>{label}<ArrowDownUp size={12}/></button>)}</div>{visibleMembers.map((member) => <div className="club-member-row" key={member.id}><label className="member-check"><input type="checkbox" aria-label={`Выбрать участника №${member.start_number}`} checked={selectedIds.has(member.id)} onChange={() => toggleMember(member.id)}/><span/></label><b className="member-number">№{member.start_number}</b><strong className="member-full-name">{member.full_name}</strong><span>{member.set_name}</span><span>{member.application_type === "collective" ? "Коллективная" : "Индивидуальная"}</span><button disabled={!canManage} className={member.checked_in ? "status-chip success" : "status-chip"} onClick={() => setPending({ scope: "individual", member, field: "checked_in", value: !member.checked_in })}>{member.checked_in ? "Прибыл" : "Не прибыл"}</button><button disabled={!canManage} className={member.is_paid ? "status-chip success" : "status-chip"} onClick={() => setPending({ scope: "individual", member, field: "is_paid", value: !member.is_paid })}>{member.is_paid ? "Оплачено" : "Не оплачено"}</button></div>)}</div>
    </div></> : <div className="no-selection"><Users size={32}/><h2>Клубов пока нет</h2><p>Они появятся автоматически из заявок участников.</p></div>}</div>
    </div>
