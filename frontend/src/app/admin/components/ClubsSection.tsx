...
import { ArrowDownUp, Check, CreditCard, PackageCheck, MoreVertical, Pencil, Search, UserCheck, Users } from "lucide-react";
...
export function ClubsSection({ token, canEdit, canMerge, canManage, onParticipantsChanged }: { token: string; canEdit: boolean; canMerge: boolean; canManage: boolean; onParticipantsChanged: () => void }) {
  ...
  return <section className="clubs-workspace">
    ...
    <aside className="clubs-list-pane"><div className="clubs-pane-head"><label className="search-box"><Search size={17}/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Найти клуб"/></label></div><div className="clubs-list">{visibleClubs.map((item) => <div className={item.id === selectedClubId ? "club-list-entry active" : "club-list-entry"} key={item.id}><button className={item.id === selectedClubId ? "club-list-item active" : "club-list-item"} onClick={() => setSelectedClubId(item.id)}><span><strong>{item.name}</strong><small>{item.representative || "Представитель не указан"}</small></span><b>{item.participant_count}</b></button>{canMerge && <div className="club-menu"><button className="club-menu-trigger" aria-label={`Действия клуба ${item.name}`} aria-expanded={menuId === item.id} onClick={() => setMenuId(menuId === item.id ? "" : item.id)}><MoreVertical size={20}/></button>{menuId === item.id && <div className="club-menu-popup"><button onClick={() => { setMergeSource(item); setMenuId(""); }}>Объединить</button></div>}</div>}</div>)}</div></aside>
