...
export function CategoriesSettings({ token, locked, onChanged, onSummary }: { onSummary?: (summary: { categories: number; participants: number }) => void; token: string; locked: boolean; onChanged: () => Promise<void> }) {
  ...
  return <div className="category-settings">
    <div className="category-relief-body">
    <div className="settings-card-head category-settings-head"><div><button className="secondary-button" disabled={locked} onClick={() => add("male")}><Plus size={15}/>Мужская</button><button className="secondary-button" disabled={locked} onClick={() => add("female")}><Plus size={15}/>Женская</button><button data-view-action className="secondary-button" onClick={() => void load()}><RotateCcw size={15}/>Сбросить</button><button className="save-route-button" disabled={locked || saving} onClick={() => void requestSave()}><Save size={16}/>{saving ? "Проверяем..." : "Проверить и сохранить"}</button></div></div>
    ...
    <div className="category-help"><div><Info size={18}/><span>0 финалистов — без финала. Квалификация требует подтверждения.</span></div><div><Medal size={18}/><span>Медаль: пустая нижняя граница — отключена; пустая верхняя — без ограничения.</span></div></div>
      <div className="category-config-table"><div className="category-config-head"><span>Категория</span><span>Пол</span><span className="category-range-head"><b>Возраст</b><small><span>от</span><span>до</span></small></span><span>В финал</span><span className="bronze-label category-range-head"><b><Medal size={18}/>Бронза</b><small><span>от</span><span>до</span></small></span><span className="silver-label category-range-head"><b><Medal size={18}/>Серебро</b><small><span>от</span><span>до</span></small></span><span className="gold-label category-range-head"><b><Medal size={18}/>Золото</b><small><span>от</span><span>до</span></small></span><span/></div>{categories.map((item, index) => <div className="category-config-row" key={item.id ?? `new-${index}`}><label data-label="Название"><input aria-label={`Название категории ${index + 1}`} value={item.name} disabled={locked} onChange={(event) => update(index, { name: event.target.value })}/></label><div className="category-cell" data-label="Пол"><select aria-label={`Пол: ${item.name}`} value={item.sex} disabled={locked} onChange={(event) => update(index, { sex: event.target.value as "male" | "female" })}><option value="male">Мужской</option><option value="female">Женский</option></select></div><div className="range-inputs age-range" data-label="Возраст: от / до"><input aria-label={`Возраст от: ${item.name}`} type="number" min="0" max="120" value={item.min_age} disabled={locked} onChange={(event) => update(index, { min_age: Number(event.target.value) })}/><input aria-label={`Возраст до: ${item.name}`} type="number" min="0" max="120" value={item.max_age ?? ""} placeholder="∞" disabled={locked} onChange={(event) => update(index, { max_age: numberValue(event.target.value) })}/></div><div className="category-cell" data-label="В финал"><input aria-label={`Количество финалистов: ${item.name}`} className="finalist-count-input" type="number" min="0" max="1000" value={item.finalist_count} disabled={locked} title="Ноль отключает финал для категории; равенство очков на границе увеличивает положительный лимит" onChange={(event) => update(index, { finalist_count: Number(event.target.value) })}/></div>{(["bronze", "silver", "gold"] as const).map((medal) => <div className="range-inputs" key={medal} data-label={`${{bronze: "Бронза", silver: "Серебро", gold: "Золото"}[medal]}: от / до`}><input aria-label={`${{bronze: "Бронза", silver: "Серебро", gold: "Золото"}[medal]} от: ${item.name}`} type="number" min="0" value={item[`${medal}_min_points`] ?? ""} placeholder="от" disabled={locked} onChange={(event) => update(index, { [`${medal}_min_points`]: numberValue(event.target.value) })}/><input aria-label={`${{bronze: "Бронза", silver: "Серебро", gold: "Золото"}[medal]} до: ${item.name}`} type="number" min="0" value={item[`${medal}_max_points`] ?? ""} placeholder="∞" disabled={locked} onChange={(event) => update(index, { [`${medal}_max_points`]: numberValue(event.target.value) })}/></div>)}<div className="category-cell"><button className="icon-button category-delete" title="Удалить категорию" disabled={locked || categories.length <= 2} onClick={() => setCategories((rows) => rows.filter((_, rowIndex) => rowIndex !== index))}><Trash2 size={15}/></button></div></div>)}</div>
    </div>
