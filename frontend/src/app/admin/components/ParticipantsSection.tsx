...
export function ParticipantsSection({
  ...
  return <section className="participants-workspace">
    <header className="admin-section-hero"><div><div className="eyebrow">Работа с фестивалем</div><h1>Участники</h1><p>{hideCheckedIn ? `${visibleParticipants.length} показано` : search.trim() ? `${participants.length} найдено по всему фестивалю` : `${participants.length} записей`}</p></div><span className="participant-toolbar-actions"><button className="secondary-button compact-action" disabled={Boolean(event?.final_started_at)} title="Добавить участника вручную" onClick={onCreate}><UserPlus size={15}/>Добавить</button><button className="import-button" disabled={Boolean(event?.final_started_at)} title="Импортировать CSV, XLSX или XLSM" onClick={onImport}><Upload size={15}/>Импорт</button></span></header>
