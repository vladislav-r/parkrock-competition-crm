...
import { ArrowDownAZ, Check, CreditCard, EyeOff, Info, LockKeyhole, MoreVertical, Pencil, Search, Plus, Upload, UserCheck, Users, X } from "lucide-react";
...
export function ParticipantsSection({
  ...
  draftCompletedRoutes, activeSetName, listSetId, onSearchChange, onSelect, onImport, onCreate, onPendingSetChange,
  ...
  function closeDetail() {
    ...
    const savedRoutes = new Set(selected?.ascents.filter(item => item.completed).map(item => item.route_id));
    if (editingResults && (savedRoutes.size !== draftCompletedRoutes.size || [...savedRoutes].some(id => !draftCompletedRoutes.has(id)))) {
  ...
  function participantActions(person: Participant, inMenu = false) {
    ...
    return <>
      ...
      <button role={inMenu ? "menuitem" : undefined} onClick={() => run(() => onReceptionAction(person.is_paid ? "unpay" : "pay"))}><CreditCard size={15}/>{person.is_paid ? "Отменить оплату" : "Подтвердить оплату"}</button></>}
  ...
  const pagination = usePagination(visibleParticipants.length, JSON.stringify([search, listSetId, hideCheckedIn, sortAlphabetically]));
  const listRef = useRef<HTMLDivElement>(null);
  useEffect(() => { listRef.current?.scrollTo({ top: 0 }); }, [pagination.offset, pagination.pageSize, search, listSetId, hideCheckedIn, sortAlphabetically]);
  ...
  return <section className="participants-workspace">
    ...
    }}><MoreVertical size={18}/></button>}</div>)}{visibleParticipants.length === 0 && <div className="participant-list-empty"><EyeOff size={24}/><strong>Прибывшие скрыты</strong><span>{hiddenCheckedInCount ? "Включите отображение, чтобы вернуть участников в список." : "В этом списке пока нет ожидающих участников."}</span></div>}</div><ListPagination {...pagination} label="Участники"/></section>
    <ResponsiveDetail open={detailOpen && Boolean(selected)} onClose={closeDetail} label="Карточка участника"><section className="participant-card">{selected ? <>
      <div className="relief-card-ribbon">Стартовый номер {selected.start_number}<button data-view-action type="button" className="participant-card-close" onClick={closeDetail} aria-label="Закрыть карточку участника" title="Закрыть карточку"><X size={18}/></button></div><div className="relief-card-body"><div className="card-head"><div><h2>{selected.surname} {selected.name}</h2><p>{selected.patronymic && <>{selected.patronymic} · </>}{selected.club}</p></div>{selected.checked_in_at ? <div className="live-score arrived"><strong>{selected.points} <small>очков</small></strong><span>Прибыл · {routeCount(selected.completed_count)}</span></div> : <span className="not-checked-in">Не пришел</span>}</div>
    ...
    {confirmClose && <ConfirmDialog title="Закрыть карточку без сохранения?" description="Изменения отметок трасс не сохранены. При закрытии они будут отменены." confirmLabel="Закрыть без сохранения" onCancel={() => setConfirmClose(false)} onConfirm={() => { setConfirmClose(false); setDetailOpen(false); onSelect(null); }}/>}
