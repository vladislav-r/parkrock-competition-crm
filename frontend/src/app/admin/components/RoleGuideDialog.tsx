...
const ROLE_GUIDES: Record<UserRole, { title: string; intro: string; actions: string[] }> = {
  ...
  route_judge: {
    ...
    actions: ["Найдите финалиста по стартовому номеру или ФИО.", "Нажмите «Старт» при начале пролаза. Отмечайте зону и топ в текущей попытке; после срыва нажмите «Попытка», перед новым пролазом — «Старт».", "Проверьте результат перед подтверждением; отправленная запись блокируется для судьи."],
  },
};

export function RoleGuideDialog({ role, onClose }: { role: UserRole; onClose: () => void }) {
  const guide = ROLE_GUIDES[role] ?? { title: role, intro: "Доступные разделы определяются правами вашей роли.", actions: ["Используйте разделы в меню для работы.", "Для изменения доступа обратитесь к администратору."] };
  return <div className="modal-backdrop role-guide-backdrop" role="presentation" onMouseDown={onClose}><section className="role-guide-dialog" role="dialog" aria-modal="true" aria-labelledby="role-guide-title" onMouseDown={(event) => event.stopPropagation()}><button data-view-action className="dialog-close" onClick={onClose} title="Закрыть"><X size={18}/></button><span className="role-guide-icon"><ShieldCheck size={25}/></span><div className="eyebrow">Краткая инструкция</div><h2 id="role-guide-title">{guide.title}</h2><p>{guide.intro}</p><div className="role-guide-steps">{guide.actions.map((action) => <div key={action}><CheckCircle2 size={18}/><span>{action}</span></div>)}</div><button data-view-action className="primary-action role-guide-continue" onClick={onClose}>Перейти к работе</button></section></div>;
}
