...
(async () => {
  ...
  try {
    ...
    await page.getByRole('heading', { name: 'Обновление результатов' }).waitFor();
    assert.equal(await page.getByLabel('Квалификация, сек.', { exact: true }).inputValue(), '30');
    assert.equal(await page.getByLabel('Финал, сек.', { exact: true }).inputValue(), '10');
