...
(async () => {
  ...
  try {
    ...
    await page.route('**/api/v1/**', async route => {
      ...
      if (url.pathname === '/api/v1/public/results') return respond({ groups: ['М 10–12', 'Женщины'], results: [], final_groups: [], stage: 'qualification' });
    ...
    const navigation = page.getByRole('navigation', { name: 'Возрастные категории' });
    assert.equal(await page.locator('.qualification-table.compact thead th').count(), 5);
    assert.equal(await page.locator('.qualification-table .bib').first().innerText(), '10');
    assert.equal(await page.locator('.qualification-table tbody td').first().evaluate(element => getComputedStyle(element).backgroundColor), 'rgb(255, 255, 255)');
    assert.equal(await navigation.getByRole('link').count(), 4);
    assert.equal(await navigation.getByRole('link', { name: 'Абсолют', exact: true }).getAttribute('aria-current'), 'page');
    await navigation.getByRole('link', { name: 'Женщины', exact: true }).click();
    await page.getByRole('heading', { name: 'Женщины', exact: true }).waitFor();
    assert.equal(await navigation.getByRole('link').count(), 4);
    await navigation.getByRole('link', { name: 'Командный зачёт', exact: true }).click();
    await page.getByRole('heading', { name: 'Скоро 😉', exact: true }).waitFor();
    assert.equal(await navigation.getByRole('link', { name: 'Командный зачёт', exact: true }).getAttribute('aria-current'), 'page');
    await navigation.getByRole('link', { name: 'Абсолют', exact: true }).click();
    await page.getByText('Иванов Александр', { exact: true }).waitFor();
    assert.equal(await page.getByRole('link', { name: 'Сеты', exact: true }).getAttribute('href'), '/sets');
